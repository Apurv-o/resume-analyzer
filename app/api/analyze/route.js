...
function buildPrompt(resumeText, jobDescription) {
  ...
  return [
    ...
    '- sectionScores: array of {name, score} 0-100 for sections like "Summary", "Experience", "Skills", "Education", "Impact language", "Structure"',
...
export async function POST(request) {
  try {
    ...
    if (file.size > MAX_FILE_BYTES) {
      return Response.json({ error: 'File is over 5MB. Please upload a smaller resume.' }, { status: 413 });
    }
    const mime = normalizeDocxType(file.type);
    ...
    const clamped = Math.max(0, Math.min(100, Math.round(data.overallScore)));
    ...
    const result = {
      ...
      sectionScores: data.sectionScores.map((s) => ({
